...
function createTransactionFromHubs(
  seed: number,
  slotIndex: number,
  sourceHub: MockHub,
  targetHub: MockHub,
...
function initialTransactions(maxTransactions: number, seedOffset: number): Transaction[] {
...
export function useLiveDashboard({
  ...
  const bootSeedRef = useRef(Math.floor(Math.random() * 1_000_000))
  const sequenceRef = useRef(bootSeedRef.current + normalizedMax)
  const [transactions, setTransactions] = useState(() => initialTransactions(normalizedMax, bootSeedRef.current))
